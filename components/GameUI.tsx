...
const GameUI: React.FC<GameUIProps> = ({
  ...
  return (
    <div className="absolute inset-0 z-10 pointer-events-none">
      ...
      {/* XO Game Modal */}
      {showXO && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 bg-white/95 backdrop-blur-xl p-4 rounded-3xl shadow-2xl border-4 border-purple-300 pointer-events-auto flex flex-col items-center animate-fade-in">
              <div className="flex justify-between w-full mb-2">
                  <h3 className="font-bold text-purple-600">Tic Tac Toe</h3>
                  <button onClick={() => setShowXO(false)}><X size={20} /></button>
              </div>
              <div className="grid grid-cols-3 gap-2 w-full aspect-square mb-4">
                  {xoState.board.map((cell, i) => (
                      <button 
                        ...
                        className={`bg-purple-50 rounded-lg text-2xl font-bold flex items-center justify-center shadow-sm ${cell === 'X' ? 'text-blue-500' : 'text-red-500'}`}
                      >
                          {cell}
                      </button>
                  ))}
              ...
              {xoState.winner && (
                  <div className="mb-2 font-bold text-xl text-green-600">
                      {xoState.winner === 'Draw' ? 'Draw!' : `Winner: ${xoState.winner}`}
                  </div>
              )}
              <button 
                onClick={() => onXOAction(null)} 
                className="flex items-center gap-2 px-4 py-2 bg-purple-500 text-white rounded-full font-bold shadow active:scale-95"
              >
                  <RotateCw size={16} /> Reset Game
              </button>
          </div>
      )}
      ...
      {/* Memory Panel */}
      {activeMemory && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-md bg-white/95 backdrop-blur-xl p-6 rounded-3xl shadow-2xl border-4 border-[#ffb7b2] pointer-events-auto flex flex-col items-center text-center animate-fade-in">
           <h3 className="text-2xl font-bold text-[#ff6b6b] mb-2">{activeMemory.title}</h3>
           {activeMemory.image && (
               <img src={activeMemory.image} alt="Memory" className="w-full h-40 object-cover rounded-lg mb-4" />
           )}
           <p className="text-gray-700 text-lg leading-relaxed mb-6">{activeMemory.text}</p>
           <button 
             onClick={() => setActiveMemory(null)}
             className="px-8 py-3 bg-[#ff6b6b] text-white font-bold rounded-full shadow-lg active:scale-95"
           >
             Close Memory
           </button>
        </div>
